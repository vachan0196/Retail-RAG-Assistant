...
def _load_index_and_meta():
    ...
    if not INDEX_PATH.exists():
        raise FileNotFoundError(f"FAISS index not found at {INDEX_PATH}. Build via `python index.py build`.")
...
def retrieve(query: str, top_k: int = 10) -> List[Hit]:
    import faiss
...
def rerank(query: str, hits: List[Hit], rerank_top_k: int = 5) -> List[Hit]:
    ...
    for h, s in zip(hits, scores):
        h.score = 1 / (1 + math.exp(-float(s)))  # sigmoid for display
    hits.sort(key=lambda h: h.score, reverse=True)
...
def answer_from_context(query: str, top_k: int = 10, rerank_top_k: int = 5) -> Tuple[str, List[Hit]]:
    ...
    use_openai = os.getenv("USE_OPENAI", "false").lower() == "true" and os.getenv("OPENAI_API_KEY")
    use_cohere = os.getenv("USE_COHERE", "false").lower() == "true" and os.getenv("COHERE_API_KEY")
    ...
    # Offline fallback: extract time expressions and build crisp answer
    time_patterns = [
    ...
    for h in hits:
        ...
        if tw:
            ...
            findings.append({"times": tw, "title": h.title, "hdr": hdr})
        if len(findings) >= 4:
            break
    ...
    if findings:
        ...
        if len(findings) > 1:
            extras = [f"- {f['times'][0]}  [{f['title']}{f['hdr']}]" for f in findings[1:]]
            if extras:
                answer += "\n\n**Related timings:**\n" + "\n".join(extras)
        return answer, hits
